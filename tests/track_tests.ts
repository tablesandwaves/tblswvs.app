...
describe("AbletonTrack", () => {
  describe("when generating rhythmic fills", () => {
    ...
    it("should the have correct number of notes", () => {
      expect(abletonNotes.length).to.eq(4);
    });
    ...
    it("should place the notes at the correct positions", () => {
      ...
      expect(abletonNotes[1].clipPosition).to.eq(0.25);
      expect(abletonNotes[2].clipPosition).to.eq(0.5);
      expect(abletonNotes[3].clipPosition).to.eq(0.75);
    })
  });
});
